...
#include "rendering/ViewportGrid.h"
...
unsigned int                             Renderer::myDebugTexture    = 0;
DebugMode                                Renderer::myDebugMode       = DebugMode::None;
...
void Renderer::Initialize()
{
    ...
    glUniform3fv(glGetUniformLocation(Renderer::myShader->GetID(), "uVoxelColors"),
                 ourVoxelColors.size(),
                 &ourVoxelColors[0][0]);
...
void Renderer::SetDebugTexture(unsigned int aTex, DebugMode aMode)
{
    myDebugTexture = aTex;
    myDebugMode    = aMode;
}
void Renderer::ClearDebugTexture()
{
    myDebugTexture = 0;
    myDebugMode    = DebugMode::None;
}
...
void Renderer::RenderFrame(Camera &aCamera, Scene &aScene, glm::ivec2 viewportSize)
{
    ...
    RenderpassInfo   info{.viewportSize          = viewportSize,
                          .framebuffers          = {}, // initialize as zero, we will add to it in the loop
                          .currentFramebufferIdx = 0,
                          .camera                = aCamera,
                          .scene                 = aScene,
                          .vertexMode            = vertexMode,
                          .debugTexture          = Renderer::myDebugTexture,
                          .debugMode             = Renderer::myDebugMode};

    size_t size = myRenderpasses.size();
    for (size_t i = 0; i < size; i++)
    {
        bool         isLast = (i == size - 1);
        Framebuffer &target = isLast ? Renderer::framebuffer : myRenderpasses[i]->framebuffer;
