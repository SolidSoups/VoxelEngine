...
StatsEditor::StatsEditor(Scene &aScene) : myScene(aScene) {}
...
void StatsEditor::Draw() {
  ...
  if(!isVoxStoneHidden) voxelMaxVal = std::max(voxelMaxVal, *std::max_element(myStoneVoxelCounts.begin(), myStoneVoxelCounts.end()));
  if(!isVoxSandHidden)  voxelMaxVal = std::max(voxelMaxVal, *std::max_element(mySandVoxelCounts.begin(), mySandVoxelCounts.end()));
  if(!isVoxWaterHidden) voxelMaxVal = std::max(voxelMaxVal, *std::max_element(myWaterVoxelCounts.begin(), myWaterVoxelCounts.end()));
  ImPlot::SetNextAxesLimits(myFrameCount >= 1000 ? myFrameCount - 1000 : 0, myFrameCount, 0, voxelMaxVal * 1.25, ImGuiCond_Always);
  if(ImPlot::BeginPlot("Voxel Count")){
    ...
    if(!isVoxStoneHidden)
      ImPlot::Annotation(myTimepoints[currentIndex], myStoneVoxelCounts[currentIndex], ImVec4(0.5, 0.5, 0.5, 1), ImVec2(10, 10), true, "%u", myStoneVoxelCounts[currentIndex]);
    ...
    if(!isVoxSandHidden)
      ImPlot::Annotation(myTimepoints[currentIndex], mySandVoxelCounts[currentIndex], ImVec4(1.0, 1.0, 0, 1), ImVec2(10, 10), true, "%u", mySandVoxelCounts[currentIndex]);
    ...
    if(!isVoxWaterHidden)
      ImPlot::Annotation(myTimepoints[currentIndex], myWaterVoxelCounts[currentIndex], ImVec4(0, 0.588, 1.0, 1), ImVec2(10, 10), true, "%u", myWaterVoxelCounts[currentIndex]);
  ...
  if(!isTriStoneHidden) triangleMaxVal = std::max(triangleMaxVal, *std::max_element(myStoneTriCounts.begin(), myStoneTriCounts.end()));
  if(!isTriSandHidden) triangleMaxVal = std::max(triangleMaxVal, *std::max_element(mySandTriCounts.begin(), mySandTriCounts.end()));
  if(!isTriWaterHidden) triangleMaxVal = std::max(triangleMaxVal, *std::max_element(myWaterTriCounts.begin(), myWaterTriCounts.end()));
  ImPlot::SetNextAxesLimits(myFrameCount >= 1000 ? myFrameCount - 1000 : 0,
                            myFrameCount, 0, triangleMaxVal * 1.25f, ImGuiCond_Always);
  if (ImPlot::BeginPlot("Triangle Count")) {
    ...
    if(!isTriStoneHidden)
      ImPlot::Annotation(myTimepoints[currentIndex], myStoneTriCounts[currentIndex], ImVec4(0.5, 0.5, 0.5, 1), ImVec2(10, 10), true, "%u", myStoneTriCounts[currentIndex]);
    ...
    if(!isTriSandHidden)
      ImPlot::Annotation(myTimepoints[currentIndex], mySandTriCounts[currentIndex], ImVec4(1.0, 1.0, 0, 1), ImVec2(10, 10), true, "%u", mySandTriCounts[currentIndex]);
    ...
    if(!isTriWaterHidden)
      ImPlot::Annotation(myTimepoints[currentIndex], myWaterTriCounts[currentIndex], ImVec4(0, 0.588, 1, 1), ImVec2(10, 10), true, "%u", myWaterTriCounts[currentIndex]);

    ImPlot::EndPlot();
  }

  // Mesh timings plot
  auto maxVal = std::max(1.0, *std::max_element(meshTimings.begin(), meshTimings.end()));
  ImPlot::SetNextAxesLimits(myFrameCount >= 1000 ? myFrameCount - 1000 : 0,
                            myFrameCount, 0, maxVal * 1.25f, ImGuiCond_Always);
  if(ImPlot::BeginPlot("Mesh Build Time")){
    ImPlot::PlotLine("Build Time", myDoubleTimepoints.data(), meshTimings.data(), count, 0, offset, sizeof(double)); 
    ImPlot::Annotation(myDoubleTimepoints[currentIndex], meshTimings[currentIndex], ImVec4(1,1,1,1), ImVec2(10, 10), true, "%.2fms", meshTimings[currentIndex]);
    ImPlot::EndPlot();
  }
