...
void BrushEditor::Draw(bool *aIsOpen)
{
    ImGui::Begin("Voxel Painter", aIsOpen);
...
void BrushEditor::DrawPalette()
{
    ...
    float totalWidth = BUTTON_COUNT * PALETTE_BUTTON_SIZE.x + (BUTTON_COUNT - 1) * ImGui::GetStyle().ItemSpacing.x +
...
void BrushEditor::DrawBrushes()
{
    ...
    float totalWidth = BRUSH_COUNT * PALETTE_BUTTON_SIZE.x + (BRUSH_COUNT - 1) * ImGui::GetStyle().ItemSpacing.x +
